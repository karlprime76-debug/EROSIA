...
import type { ScoringEngine, BehaviorInput, BehaviorOutput } from './types'
...
export class BehaviorEngine implements ScoringEngine<BehaviorInput, BehaviorOutput> {
  ...
  async compute(input: BehaviorInput): Promise<BehaviorOutput> {
    return computeBehavior(input.userId)
  }
}

async function computeBehavior(userId: string): Promise<BehaviorOutput> {
  ...
  const daysActive = new Set((views.data ?? []).map(v =>
  ...
  const allActions7d = await supabase
    .from('behavior_log')
    .select('created_at')
    .eq('user_id', userId)
    .gte('created_at', new Date(Date.now() - SEVEN_DAYS).toISOString())

  const activeDays = new Set((allActions7d.data ?? []).map(a =>
  ...
  const sent = (msgs.data ?? []).filter(a => (a as Record<string, string>).action === 'send_message').length
  const replied = (msgs.data ?? []).filter(a => (a as Record<string, string>).action === 'reply_message').length
  signals.replyRate = sent > 0 ? replied / sent : 0

  // Diversité des actions
  const distinctActions = new Set((allActions7d.data ?? []).map(a => (a as Record<string, string>).action)).size
