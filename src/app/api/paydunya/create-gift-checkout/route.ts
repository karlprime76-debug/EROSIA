...
export async function POST(request: Request) {
  try {
    ...
    if (!gift) return apiError('Cadeau introuvable', 404)
    if (typeof gift.price_cents !== 'number') {
      ...
      return apiError('Prix du cadeau invalide', 500)
    }
    ...
    if (result.status !== 'success' || !result.token) {
      ...
      return apiError('Erreur de création du paiement. Contacte le support si le problème persiste.', 500)
    }
