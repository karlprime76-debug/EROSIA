...
export async function POST(request: NextRequest) {
  try {
    ...
    const tables = [
      ...
      ['duels',           () => admin.from('duels').delete().or(`creator_id.eq.${uid},profile_a_id.eq.${uid},profile_b_id.eq.${uid}`)],
      ['duel_votes',      () => admin.from('duel_votes').delete().or(`voter_id.eq.${uid},chosen_id.eq.${uid}`)],
