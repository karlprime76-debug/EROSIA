  ...
  return (
    <div className="bg-transparent flex-1 flex flex-col px-8">
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <p className="text-base text-center text-[#9E9488] leading-relaxed max-w-sm">{s.desc}</p>
      </div>
      <div className="pb-16 flex flex-col items-center gap-6">
        <div className="flex gap-2">
          {slides.map((_, i) => (
            <div key={i} className={`w-2 h-2 rounded-full ${i === slide ? 'w-6 bg-[#D92D4A]' : 'bg-[#2A2826]'}`} />
          ))}
        </div>
        <button onClick={next} className="w-full max-w-sm py-3.5 rounded-full text-white font-semibold" style={{ background: '#D92D4A' }}>
          {slide === slides.length - 1 ? 'Créer mon compte' : 'Suivant'}
        </button>
        {slide < slides.length - 1 && (
          <button onClick={() => router.push('/register')} className="text-[#9E9488] text-sm">Passer</button>
        )}
      </div>
    </div>
  )
