...
-- 6. Trust Score column for profiles
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='trust_score') THEN
    ALTER TABLE profiles ADD COLUMN trust_score INT DEFAULT 50;
    ALTER TABLE profiles ADD CONSTRAINT trust_score_check CHECK (trust_score BETWEEN 0 AND 100);
  END IF;
END $$;

-- 7. Energy Score column for profiles
DO $$ BEGIN
